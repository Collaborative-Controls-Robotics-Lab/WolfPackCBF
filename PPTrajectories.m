...
xp = sortrows(xp.', 2).';
%xp = [0.4*ones(1,N); linspace(-0.05, 0.05, N); pi*ones(1,N)];
...
for qq = 1:T
    ...
    for ii = 1:N
        ...
        He{ii} = zeros(2);
        ke{ii} = Aie(ii,:).';
        bie(ii) = -vmax + (norm(xe(1:2) - xp(1:2,ii)) - 1.05*epsilon);
        de{ii} = -bie(ii);
    end
    uvmax = vmax/velRatio;
    He{N+1} = zeros(2);
    ...
    for ii = 1:N
        ...
        if norm(xp(1:2,ii) - xe(1:2)) <= epsilon
            xunit = epsilon*cos(th) + xe(1);
            yunit = epsilon*sin(th) + xe(2);
            plot(xunit, yunit, 'k');
        end
    end
    xeva(qq,:) = xe(1:2).';
