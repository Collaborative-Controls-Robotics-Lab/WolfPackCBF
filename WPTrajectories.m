...
% State Vectors
% Pursuers
xp = [0.55*ones(1,N); linspace(-0.15, 0.15, N); pi*ones(1,N)];
...
th = 0:pi/50:2*pi;
xunit = 0.02*cos(th) + Pxavg;
yunit = 0.02*sin(th) + Pyavg;
fill(xunit, yunit, 'g');
xlim([-0.45 0.57]);
ylim([-0.25 0.25]);
% Evader
...
%h1 = patch(xs, ys, 'red');
plot(xe(1), xe(2), 'r', 'LineStyle', 'none', 'LineWidth', 2);
% Pursuers
%[xs,ys] = getpatches(xp,0.05);
plot(xp(1,:), xp(2,:), 'b', 'LineStyle', 'none', 'LineWidth', 2);
%h2 = patch(xs,ys, 'blue');
...
for qq = 1:T
    ...
    for ii = 1:N
        ...
        He{ii} = zeros(2);
        ke{ii} = Aie(ii,:).';
        bie(ii) = -vmax + (norm(xe(1:2) - xp(1:2,ii)) - 1.05*0.05);
        de{ii} = -bie(ii);
    end
    uvmax = vmax/velRatio;
    He{N+1} = zeros(2);
    ...
    for ii = 1:N
        ...
        elseif ii == 1
            ...
            if (xe(1) + 0.05 > xp(1,ii))
                z = ui/norm(ui)*vmax;
            else
            Aij = -(velRatio*(xp(1:2,ii) - xe(1:2)).'/norm(xp(1:2,ii) - xe(1:2)) ...
                - (xp(1:2,ii) - xp(1:2,ii+1)).'/norm(xp(1:2,ii) - xp(1:2,ii+1)));
            bij = a1/(a1 + a2)*(velRatio*((xe(1:2) - xp(1:2,ii)).'/norm(xe(1:2) - xp(1:2,ii)) + (xe(1:2) - xp(1:2,ii+1)).'/norm(xe(1:2) - xp(1:2,ii+1))) ...
                * ue + 1e4*(velRatio*(norm(xp(1:2,ii) - xe(1:2)) + norm(xp(1:2,ii+1) - xe(1:2))) - norm(xp(1:2,ii) - xp(1:2,ii+1)) + epsilon)^3);
            % A1 - CRS constraint
            [x,y,lambda] = closestEllipse(CircValues(1:2,ii+1), xe(1:2), [Pxavg; Pyavg], (T-qq)*dt*vmax/velRatio);
            [A,B,C,D,E,F] = ellipseData(xe(1:2), [Pxavg; Pyavg], (T-qq)*dt*vmax/velRatio);
            xstar = [x;y];
            dxi = dxstardxi(A,B,C,D,E,F,x,y,lambda,velRatio);
            dxe = dxstardxe(A,B,C,D,E,F,x,y,lambda,velRatio,[Pxavg;Pyavg],xe(1:2),(T-qq)*dt*vmax/velRatio);
            dxt = dxstardt(A,B,C,D,E,F,x,y,lambda,vmax/velRatio,[Pxavg;Pyavg],xe(1:2),(T-qq)*dt*vmax/velRatio);
            num = xp(1:2,ii) - xstar + velRatio^2*xstar - velRatio^2*xe(1:2);
            ...
            b1 = dhdxe*ue + dhdxstar*(dxe*ue + dxt) + 1e3*(velRatio*norm(xie) - norm(num) + epsilon)^3;
            A = [Aij; A1]; 
            b = [bij; b1];
            ...
            if (size(z,1) == 0)
                %z = (xe(1:2) - xp(1:2,ii))/norm(xe(1:2) - xp(1:2,ii))*vmax;
                z = ui/norm(ui)*vmax;
                cvxflag = 1;
            elseif isnan(z) ~= [0;0]
                z = ui/norm(ui)*vmax;
        ...
        elseif ii == N
            ...
            Aij(1,:) = -(velRatio*(xp(1:2,ii) - xe(1:2)).'/norm(xp(1:2,ii) - xe(1:2)) ...
                - (xp(1:2,ii) - xp(1:2,ii+1)).'/norm(xp(1:2,ii) - xp(1:2,ii+1)));
            Aij(2,:) = -(velRatio*(xp(1:2,ii) - xe(1:2)).'/norm(xp(1:2,ii) - xe(1:2)) ...
                - (xp(1:2,ii) - xp(1:2,ii-1)).'/norm(xp(1:2,ii) - xp(1:2,ii-1)));
            bij(1) = a1/(a1 + a2)*(velRatio*((xe(1:2) - xp(1:2,ii)).'/norm(xe(1:2) - xp(1:2,ii)) + (xe(1:2) - xp(1:2,ii+1)).'/norm(xe(1:2) - xp(1:2,ii+1))) ...
                * ue + 1e4*(velRatio*(norm(xp(1:2,ii) - xe(1:2)) + norm(xp(1:2,ii+1) - xe(1:2))) - norm(xp(1:2,ii) - xp(1:2,ii+1)) + epsilon)^3);
            bij(2) = a1/(a1 + a0)*(velRatio*((xe(1:2) - xp(1:2,ii)).'/norm(xe(1:2) - xp(1:2,ii)) + (xe(1:2) - xp(1:2,ii-1)).'/norm(xe(1:2) - xp(1:2,ii-1))) ...
                * ue + 1e4*(velRatio*(norm(xp(1:2,ii) - xe(1:2)) + norm(xp(1:2,ii-1) - xe(1:2))) - norm(xp(1:2,ii) - xp(1:2,ii-1)) + epsilon)^3);
            %z = quadprog(eye(2), -ui, Aij, bij, [], [], ones(2,1)*-vmax/sqrt(2), ones(2,1)*vmax/sqrt(2));
            %cvx_begin quiet
            %    variable z(2)
            %    minimize( (ui - z).' * (ui - z))
            %    subject to
    ...
    for ii = 1:N
        ...
        if norm(xp(1:2,ii) - xe(1:2)) <= 0.03
            xunit = 0.03*cos(th) + xe(1);
            yunit = 0.03*sin(th) + xe(2);
            plot(xunit, yunit, 'k');
        end
    end
    xeva(qq,:) = xe(1:2).';
    ...
    %plot(xe(1), xe(2), 'r*', 'LineStyle', 'none', 'LineWidth', 0.2);
    %plot(xp(1,:), xp(2,:), 'b*', 'LineStyle', 'none', 'LineWidth', 0.2);
    %set(h2, 'XData', xps, 'YData', yps);
    drawnow limitrate
    %pause(0.05)
    for ii = 1:N
        if norm([xe(1); xe(2)] - [xp(1,ii); xp(2,ii)]) <= 0.03
            flag = 1;
        end
    end
...
plot(xpur(:,1:2:end), xpur(:,2:2:end), 'LineWidth', 2);
plot(xeva(:,1), xeva(:,2), 'LineWidth', 2);
if vid == true
    close(vidObj);
end
%figure(2);
