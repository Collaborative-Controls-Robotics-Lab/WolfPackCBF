...
for qq = 1:T
    ...
    if initializing
        ...
        if ~achievedInitialPosition
            % Go to goal
            dxi = controlScaleFactor*positionController(robotXY,[initialPoseX; initialPoseY]);
...
%     dxu(1,[dxu(1,1:numPursuers)>vmax false]) = vmax;
%     dxu(1,[dxu(1,1:numPursuers)<-vmax false])= -vmax;
%     dxu(1,[false(1,numPursuers) dxu(1,numRobots)>vmax/vratio]) = vmax/vratio;
%     dxu(1,[false(1,numPursuers) dxu(1,numRobots)<-vmax/vratio]) = -vmax/vratio;
...
%     dxu(2,dxu(2,:)<-angularVelLimit) = -angularVelLimit;
    ...
    % Perform numerical integration for pursuers, record pursuer positions
    for ii = 1:N
    ...
    % Perform numerical integration for evaders, record evaders positions
    for jj = 1:M
        xeva(qq,:) = xe(1:2,jj).';
